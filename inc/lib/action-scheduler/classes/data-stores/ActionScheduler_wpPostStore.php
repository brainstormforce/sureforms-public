...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	public function query_actions( $query = array(), $query_type = 'select' ) {
		...
		return ( 'count' === $query_type ) ? $wpdb->get_var( $sql ) : $wpdb->get_col( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- SQL generated by internal method
	}
	...
	protected function claim_actions( $claim_id, $limit, DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		$rows_affected = $wpdb->query( $wpdb->prepare( "{$update} {$where} {$order}", $params ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare,PluginCheck.Security.DirectDB.UnescapedDBParameter -- SQL fragments generated internally
	...
	private function get_post_column( $action_id, $column_name ) {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				"SELECT {$column_name} FROM {$wpdb->posts} WHERE ID=%d AND post_type=%s", // phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter -- column/table from safe source
				$action_id,
				self::POST_TYPE
			)
		);
	...
	public function migration_dependencies_met( $setting ) {
		...
		if ( empty( $dependencies_met ) ) {
			...
			$found_action        = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching,PluginCheck.Security.DirectDB.UnescapedDBParameter -- table from $wpdb object
				$wpdb->prepare(
