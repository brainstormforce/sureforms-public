...
class Block extends Base {
	...
	public function render( $attributes, $content = '' ) {
		...
		<div class="sureforms-number-slider-container main-container frontend-inputs-holder <?php echo esc_attr( $classname ); ?>">
			...
			<div style="font-size:14px; font-weight:600;" class="text-primary"><?php echo esc_attr( $value_display_text ); ?><span id="sureforms-number-slider-value-<?php echo esc_attr( $id ); ?>">0</span></div>
			<?php echo '' !== $help ? '<label class="text-secondary sforms-helper-txt" for="sureforms-number-slider-' . esc_attr( $id ) . '">' . esc_attr( $help ) . '</label>' : ''; ?>
