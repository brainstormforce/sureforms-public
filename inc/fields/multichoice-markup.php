		...
		if ( is_array( $options ) ) {
			foreach ( $options as $i => $option ) {
						...
						<div class="flex items-start classic-radio-btn sf-classic-multi-choice">
							...
							<div> 
								<article id="multi-choice-option-' . esc_attr( $block_id . '-' . $i ) . '" class="text-sm font-medium leading-6 text-primary_color mt-[-0.5px]">' . esc_html( $option['optiontitle'] ) . '</article>
							</div>
						</div>
						</label>';
			}
