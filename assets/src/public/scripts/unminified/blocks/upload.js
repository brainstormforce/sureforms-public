function initializeUploadField() {
	const uploadFieldsContainer = document.querySelectorAll('.srfm-upload-block');
				...
				uploadInput.addEventListener('change', (e) => {
					...
					if (file) {
						if (file.size > maxFileSize) {
							...
							const fileName = file.name.length > 20 ? file.name.substring(0, 17) + '...' + file.name.split('.').pop() : file.name;
							...
							reader.onload = function (event) {
								...
								const uploadResultContainer = element.querySelector('.srfm-upload-data');
								...
								<div class="srfm-upload-data-left">
									...
									<div class="srfm-upload-data-details">
										...
										<div class="srfm-upload-data-size"> ${( file.size / 1000000 ).toFixed(2)}MB</div>
									</div>
								</div>
								...
								if (uploadResultContainer) {
									uploadResultContainer.classList.add('active');
								}
								const resetButton =
									uploadResultContainer.querySelector('.srfm-upload-data-delete');
								if (resetButton) {
									resetButton.addEventListener(
										...
										() => {
											...
											if (uploadResultContainer) {
												uploadResultContainer.classList.remove('active');
											}
										}
									);
								}
