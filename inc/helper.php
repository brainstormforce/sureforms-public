...
class Helper {
	...
	public static function generate_common_form_markup( $form_id, $type, $label = '', $slug = '', $block_id = '', $required = false, $help = '', $error_msg = '', $is_unique = false, $duplicate_msg = '', $override = false ) {
		...
		switch ( $type ) {
			case 'label':
				$markup = $label ? '<label for="srfm-' . $slug . '-' . esc_attr( $block_id ) . '" class="srfm-block-label">' . htmlspecialchars_decode( esc_html( $label ) ) . ( $required ? '<span class="srfm-required" aria-label="' . __( 'Required', 'sureforms' ) . '"> *</span>' : '' ) . '</label>' : '';
				break;
			case 'help':
