jQuery(document).ready(function($) {
    ...
    $.post(ajaxurl, {
        action: 'sureforms_should_show_pointer'
    }, function(response) {
        if (!response || !response.show) {
            return;
        }

        var $target = $('#toplevel_page_sureforms_menu');
        if (!$target.length) {
            $target = $('#menu-plugins'); // fallback
        }

        var pointerContent = '<h3>' + response.title + '</h3>' +
        ...
        $target.pointer({
            ...
            buttons: function(event, t) {
                var dismissBtn = $('<a class="close" href="#" style="margin-left:8px;"></a>')
                    .text(wp.i18n.__('Dismiss', 'sureforms'))
                    .on('click.pointer', function(e) {
                        e.preventDefault();
                        t.element.pointer('close');
                        $.post(ajaxurl, { action: 'sureforms_dismiss_pointer' });
                    });

                var ctaBtn = $('<a class="button button-primary" href="' + response.button_url + '" style="margin-right:8px;">' + response.button_text + '</a>')
                    .on('click.pointer', function(e) {
                        t.element.pointer('close');
                    });
