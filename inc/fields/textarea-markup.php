...
class Textarea_Markup extends Base {
	...
	public function default_styling( $attributes ) {
		...
		'<label for="sureforms-textarea-' . esc_attr( $block_id ) . '" class="sf-text-primary">' . esc_html( $label ) . ' ' .
		...
		'<textarea style="width:100%" name="' . esc_attr( str_replace( ' ', '_', $label . 'SF-divider' . $block_id ) ) . '" id="sureforms-textarea-' . esc_attr( $block_id ) . '" aria-required="' . esc_attr( $required ? 'true' : 'false' ) . '" placeholder="' . esc_attr( $placeholder ) . '" maxLength="' . ( 0 === $max_length ? '' : esc_attr( $max_length ) ) . '" cols="' . esc_attr( $cols ) . '" rows="' . esc_attr( $rows ) . '" class="sureforms-textarea-field">' . esc_attr( $default ) . '</textarea></div>' .
	...
	public function classic_styling( $attributes ) {
		...
		<div class="frontend-inputs-holder main-container sureforms-textarea-container ' . esc_attr( $classname ) . '">
			<label for="sureforms-textarea-' . esc_attr( $block_id ) . '" class="sf-classic-label-text">
				' . esc_html( $label ) . '
			...
			<div class="mt-2 relative">
				...
				<textarea name="' . esc_attr( str_replace( ' ', '_', $label . 'SF-divider' . $block_id ) ) . '"
					...
					id="sureforms-textarea-' . esc_attr( $block_id ) . '" class="sf-classic-textarea-element">' . esc_html( $default ) . '</textarea>
			</div>
