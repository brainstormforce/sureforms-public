			...
			<label for="text" class="sf-classic-label-text"><?php echo esc_html( $label ); ?> <?php echo $required && $label ? '<span class="text-red-500"> *</span>' : ''; ?></label>
				...
				<?php foreach ( $options as $i => $option ) : ?>
					...
					<div class="flex items-center classic-radio-btn sf-classic-multi-choice">
						...
						<div>
							<article id="multi-choice-option-<?php echo esc_attr( $id . '-' . $i ); ?>" class="text-sm font-medium leading-6 text-sf_primary_color"><?php echo esc_html( $option ); ?></article>
						</div>
					</div>
					</label>
				<?php endforeach; ?>
				</div>
				<?php echo '' !== $help ? '<p class="sforms-helper-txt">' . esc_html( $help ) . '</p>' : ''; ?>
