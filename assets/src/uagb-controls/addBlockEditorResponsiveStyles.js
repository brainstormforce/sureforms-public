const addBlockEditorResponsiveStyles = (
	clientId,
	styling,
	deviceTypeClass
) => {
	// Desktop.
	const findResponsiveElement = document.getElementById(
		`block-${ clientId }`
	);
	...
	if ( 0 !== tabletPreview.length || 0 !== mobilePreview.length ) {
		...
		const findResponsiveElementInIframe = iframeDocument.getElementById(
			`block-${ clientId }`
		);

		if (
			null !== findResponsiveElementInIframe &&
			undefined !== findResponsiveElementInIframe
		) {
			styling.map( ( item ) => {
				if ( item ) {
					findResponsiveElementInIframe.classList.remove(
						item,
						deviceTypeClass
					);
					if (
