...
class Config {
	...
	public function get_source_store() {
		if ( empty( $this->source_store ) ) {
			throw new \RuntimeException( esc_html( __( 'Source store must be configured before running a migration', 'action-scheduler' ) ) );
		}
	...
	public function get_source_logger() {
		if ( empty( $this->source_logger ) ) {
			throw new \RuntimeException( esc_html( __( 'Source logger must be configured before running a migration', 'action-scheduler' ) ) );
		}
	...
	public function get_destination_store() {
		if ( empty( $this->destination_store ) ) {
			throw new \RuntimeException( esc_html( __( 'Destination store must be configured before running a migration', 'action-scheduler' ) ) );
		}
	...
	public function get_destination_logger() {
		if ( empty( $this->destination_logger ) ) {
			throw new \RuntimeException( esc_html( __( 'Destination logger must be configured before running a migration', 'action-scheduler' ) ) );
		}
