...
const GlobalBlockStyles = ( props ) => {
	...
	const currentBlockDefaultAttributes =
		allBlocksAttributes[ blockNameStripped ];

	// Check page context for FSE template.
	const pageContext = select( 'core/edit-site' )?.getPage();
	const pageTemplateSlug = pageContext?.context?.templateSlug
		? pageContext.context.templateSlug
		: null;
	...
	const globalBlockStyleOptions = getGlobalBlockStylesOptions(
		globalBlockStyles,
		blockName
	);
	...
	const saveStylesToDatabase = (
		bulkUpdateStyles = 'no',
		spectraGlobalStyles = globalBlockStyles,
		noneValue = false
	) => {
		let styleAttributes = {};
		...
		spectraGlobalStyles.map( ( style ) => {
			if (
				style?.value === uniqueID ||
				style?.value === globalBlockStyleId
			) {
		...
		formData.append(
			'globalBlockStylesFontFamilies',
			JSON.stringify( globalBlockStylesFontFamilies )
		);
	...
	const generateBlockStyles = ( newStyleID = globalBlockStyleId ) => {
		...
		globalBlockStyles.map( ( style ) => {
			if ( newStyleID && style?.value === String( newStyleID ) ) {
				...
				if ( styleForGlobal ) {
					style.styleForGlobal = updatePostIdInGbsArray(
						style,
						'widget',
						'widget_ids'
					);
	...
	const manageGbsOptions = applyFilters(
		`spectra.manage.gbs.options`,
		'',
		props
	);
	...
	return (
		<UAGAdvancedPanelBody
			title={ __( 'Global Block Styles', 'sureforms' ) }
			initialOpen={ true }
					...
					onChange={ ( value ) => {
						...
						globalBlockStyles.map( ( style ) => {
							if ( style?.value === value ) {
								label = style?.label;
								// Save the Post IDs of the Pages where GBS is used.
								style.post_ids = updatePostIdInGbsArray(
									style,
									currentPostID
								);
...
export default compose(
	withSelect( ( spectraGbsSelect ) => {
		const globalBlockStyles =
			spectraGbsSelect( storeName ).getGlobalBlockStyles();
		const isOpen =
			spectraGbsSelect( storeName ).getGlobalBlockStylesPopupState();
	...
	withDispatch( ( spectraGbsDispatch ) => ( {
		openModal: () =>
			spectraGbsDispatch( spectraStore ).toggleGlobalBlockStylesPopup(
				'open'
			),
		closeModal: () =>
			spectraGbsDispatch( spectraStore ).toggleGlobalBlockStylesPopup(
		...
		updateGlobalBlockStyles: ( value ) =>
			spectraGbsDispatch( spectraStore ).updateGlobalBlockStyles( value ),
