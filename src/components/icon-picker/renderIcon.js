...
function renderSVG( svg, setAttributes = false ) {
	...
	if ( 0 !== srfm_spec_blocks_info.font_awesome_5_polyfill.length ) {
		fontAwesome = srfm_spec_blocks_info.uagb_svg_icons[ svg ];
		if ( ! fontAwesome ) {
			fontAwesome =
				srfm_spec_blocks_info.uagb_svg_icons[
					srfm_spec_blocks_info.font_awesome_5_polyfill?.[ svg ]
				];
		}
	}
	...
	const fontAwesomeSvg = fontAwesome.svg?.brands
	...
	switch ( svg ) {
		...
		case 'align-justify':
			align = {
				d: 'M0 0H18V2H0V0ZM0 5.00001H18V7.00001H0V5.00001ZM0 10H18V12H0V10Z',
			};
			break;
	}
