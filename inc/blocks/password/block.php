...
class Block extends Base {
	...
	public function render( $attributes, $content = '' ) {
		...
		<div class="sureforms-input-password-container sf-classic-inputs-holder main-container <?php echo esc_attr( $classname ); ?>">
			<label for="sureforms-input-password-<?php echo esc_attr( $id ); ?>" class="block text-sm font-medium leading-6 text-primary_color">
			...
			<div class="relative mt-2">
				...
				class="block !w-full !border-solid !border-0 !border-[#D1D5DB] !rounded-md !py-1.5 !pr-10 !text-gray-900 !shadow-sm !ring-1 !ring-inset  !ring-gray-300 !bg-white placeholder:!text-gray-500 focus:!ring-2 focus:!border-solid focus:!border-primary_color focus:!ring-primary_color focus:!outline-0 focus:!bg-white sm:text-sm sm:leading-6">
			...
			<?php
			echo true === $is_confirm_password
			? '<label for="sureforms-input-password-' . esc_attr( $id ) . '" class="block text-sm font-medium leading-6 text-primary_color !mt-[24px]">' .
				...
				' class="sureforms-confirm-input-password block !w-full !border-solid !border-0 !border-[#D1D5DB] !rounded-md !py-1.5 !pr-10 !text-gray-900 !shadow-sm !ring-1 !ring-inset  !ring-gray-300 !bg-white placeholder:!text-gray-500 focus:!ring-2 focus:!border-solid focus:!border-primary_color focus:!ring-primary_color focus:!outline-0 focus:!bg-white sm:text-sm sm:leading-6">' .
				'<div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">' .
				'<svg  class="h-5 w-5 text-gray-400" viewBox="0 0 30 30" fill="currentColor" aria-hidden="true">' .
				'<path d="M 15 2 C 11.145666 2 8 5.1456661 8 9 L 8 11 L 6 11 C 4.895 11 4 11.895 4 13 L 4 25 C 4 26.105 4.895 27 6 27 L 24 27 C 25.105 27 26 26.105 26 25 L 26 13 C 26 11.895 25.105 11 24 11 L 22 11 L 22 9 C 22 5.2715823 19.036581 2.2685653 15.355469 2.0722656 A 1.0001 1.0001 0 0 0 15 2 z M 15 4 C 17.773666 4 20 6.2263339 20 9 L 20 11 L 10 11 L 10 9 C 10 6.2263339 12.226334 4 15 4 z"></path>' .
				'</svg>' .
			...
			?>
			<p style="display:none" class="error-message !mt-2 text-sm text-red-600" id="email-error"><?php echo esc_html( $error_msg ); ?></p>
			<p style="display:none" class="error-message confirm-password-error !mt-2 text-sm text-red-600"><?php echo esc_html( __( 'Password does not match', 'sureforms' ) ); ?></p>
		</div>
			<?php
		}
			return ob_get_clean();
