...
const AddNewPopupStyle = ( props ) => {
	...
	const onClickSaveButton = () => {
		...
		if ( ! tempStyleNameTrimmed || '' === tempStyleNameTrimmed ) {
			setLabelError( __( 'Please enter a style name', 'sureforms' ) );
			return;
		}

		// check label should be more then 3 characters and less then 50 characters.
		if (
			tempStyleNameTrimmed.length < 3 ||
			tempStyleNameTrimmed.length > 50
		) {
			setLabelError(
				__(
					'Style name should be more then 3 characters and less then 50 characters',
					'sureforms'
				)
			);
			return;
		...
		if (
			globalBlockStyles.find(
				( style ) =>
					style.label === tempStyleNameTrimmed &&
					style.blockName === blockName
			)
		) {
			setLabelError( __( 'Style name already exists', 'sureforms' ) );
			return;
		}
		...
		if ( hasSpecialCharacter.test( tempStyleNameTrimmed ) ) {
			setLabelError(
				__(
					'Style name should not contain special characters',
					'sureforms'
				)
			);
			return;
		...
		const styleNameClass =
			tempStyleNameTrimmed?.replace( /\s+/g, '-' )?.toLowerCase() +
			uniqueID;

		setAttributes( {
			globalBlockStyleName: tempStyleNameTrimmed,
			globalBlockStyleId: styleNameClass,
		} );
				...
				>
					<div className="gbs-add-new-style-head">
						<div>
							...
							<p>
								{ ' ' }
								{ __(
									'Use the saved style anywhere on the website',
									'sureforms'
								) }
							</p>
						</div>
					...
					<div
						className={ `input-wrap ${
							labelError ? 'gbs-error' : ''
						}` }
					>
						<UAGTextControl
							placeholder={ __( 'Name', 'sureforms' ) }
							value={ tempStyleName }
							onChange={ ( value ) => {
								setTempStyleName( value?.toString() );
							} }
							label={ __( 'Style Name', 'sureforms' ) }
							onKeyUp={ ( event ) => {
						...
						/>
						{ labelError && (
							<p className="gbs-error-msg">{ labelError }</p>
						) }
					</div>
