	...
	class BSF_Analytics {
		...
		public function load_deactivation_survey_form() {
			...
			if ( class_exists( 'Deactivation_Survey_Feedback' ) ) {
				foreach ( $this->entities as $key => $data ) {
					...
					if ( ! empty( $data['deactivation_survey'] ) ) {
						Deactivation_Survey_Feedback::show_feedback_form(
							$data['deactivation_survey']
						);
					}
				}
			}
		...
		public function add_slugs_to_uds_vars( $vars ) {
			foreach ( $this->entities as $key => $data ) {
				...
				if ( isset( $data['deactivation_survey'] ) && isset( $data['deactivation_survey']['source'] ) && ! empty( $data['deactivation_survey']['source'] ) ) {
					$vars['_plugin_slug'] = isset( $vars['_plugin_slug'] ) ? array_merge( $vars['_plugin_slug'], array( $data['deactivation_survey']['source'] ) ) : array( $data['deactivation_survey']['source'] );
				}
			}
