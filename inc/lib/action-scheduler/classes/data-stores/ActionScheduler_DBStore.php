...
class ActionScheduler_DBStore extends ActionScheduler_Store {
	...
	private function save_action_to_db( ActionScheduler_Action $action, DateTime $date = null, $unique = false ) {
		...
		try {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $insert_sql is already prepared internally
...
WHERE ( $where_clause ) IS NULL",
	...
	private function build_where_clause_for_insert( $data, $table_name, $unique ) {
		...
		// phpcs:disable WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- $pending_status_placeholders is hardcoded, PHPCS cannot count dynamic placeholders
...
",
			...
			switch ( $query['partial_args_matching'] ) {
				case 'json':
					...
					foreach ( $query['args'] as $key => $value ) {
						...
						$sql         .= ' AND JSON_EXTRACT(a.args, %s)=' . $placeholder; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- placeholder type is hardcoded
						$sql_params[] = '$.' . $key;
						$sql_params[] = $value;
					}
					break;
	...
	public function query_actions( $query = array(), $query_type = 'select' ) {
		...
		return ( 'count' === $query_type ) ? $wpdb->get_var( $sql ) : $wpdb->get_col( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.NoSql, WordPress.DB.DirectDatabaseQuery.NoCaching,PluginCheck.Security.DirectDB.UnescapedDBParameter -- SQL generated by internal method
	}
	...
	public function action_counts() {
		...
		foreach ( $wpdb->get_results( $sql ) as $action_data ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- SQL generated by internal method
	...
	protected function bulk_cancel_actions( $query_args ) {
		...
		while ( $action_ids ) {
			...
			$wpdb->query(
				$wpdb->prepare(
					"UPDATE {$wpdb->actionscheduler_actions} SET status = %s WHERE action_id IN {$query_in}", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $query_in built from hardcoded %d placeholders
					$parameters
				)
			);
	...
	protected function claim_actions( $claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		$sql           = $wpdb->prepare( "{$update} {$where} {$order} LIMIT %d", $params ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders,PluginCheck.Security.DirectDB.UnescapedDBParameter -- SQL fragments generated internally
		$rows_affected = $wpdb->query( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching,PluginCheck.Security.DirectDB.UnescapedDBParameter -- SQL already prepared above
	...
	public function release_claim( ActionScheduler_ActionClaim $claim ) {
		...
		if ( count( $action_ids ) > 0 ) {
			...
			$row_updates      = $wpdb->query( "UPDATE {$wpdb->actionscheduler_actions} SET claim_id = 0 WHERE action_id IN ({$action_id_string})" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $action_id_string built with absint
		}
