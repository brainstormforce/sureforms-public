...
export function getLabel( globalBlockStyleId ) {
	return ! globalBlockStyleId
		? __( 'Link to Existing Style', 'sureforms' )
		: __( 'Linked Style', 'sureforms' );
}

export function getGlobalBlockStylesOptions( globalBlockStyles, blockName ) {
	return globalBlockStyles.filter(
		( style ) => style?.blockName === blockName || ! style?.blockName
	);
}

export function clearCurrentAttributes(
...
export function updatePostIdInGbsArray(
	style,
	store_ids,
	id_or_slug = 'post_ids'
) {
	const idsArray =
		'post_ids' === id_or_slug ? 'post_ids' : 'page_template_slugs';
...
//
export const currentGlobalBlockStyleIdAttributes = (
	globalBlockStyles,
	globalBlockStyleId
) => {
	const getGbsValue = globalBlockStyles.find(
		( style ) => style?.value === globalBlockStyleId
	);
