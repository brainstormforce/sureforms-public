...
class Block extends Base {
	...
	public function render( $attributes, $content = '' ) {
		if ( ! empty( $attributes ) ) {
			...
			ob_start(); ?>
		<div class="sureforms-input-password-container main-container" style="display:flex; flex-direction:column; gap:0.5rem;">
			<label for="sureforms-input-password" class="text-primary"><?php echo esc_attr( $label ); ?> 
			...
			">
			<?php echo '' !== $help ? '<label for="sureforms-input-password" class="text-secondary">' . esc_attr( $help ) . '</label>' : ''; ?>
		</div>
			<?php
		}
			return ob_get_clean();
