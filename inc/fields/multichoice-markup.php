			...
			foreach ( $options as $i => $option ) {
				$output .= '	<div style="display: flex; align-items: center; gap:4px">
					<input
						...
						id="sureforms-multi-choice-' . esc_attr( $id . '-' . $i ) . '"
						type="' . esc_attr( $single_selection ? 'radio' : 'checkbox' ) . '"
						' . esc_attr( $single_selection ? 'name="' . esc_attr( "sf-radio-$id" ) . '"' : '' ) . '
					/>
					<label
	...
	public function classic_styling( $attributes ) {
		...
		if ( is_array( $options ) ) {
			foreach ( $options as $i => $option ) {
						...
						<input type="' . esc_attr( $single_selection ? 'radio' : 'checkbox' ) . '" ' . esc_attr( $single_selection ? 'name="' . esc_attr( "sf-radio-$id" ) . '"' : '' ) . ' id="sureforms-multi-choice-' . esc_attr( $id . '-' . $i ) . '" class="sureforms-multi-choice">
