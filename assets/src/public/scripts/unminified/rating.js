...
function initializeRatings() {
    ...
    const classicRatingContainers = document.getElementsByClassName('srfm-classic-rating-container');

    for (const classicRating of classicRatingContainers) {
        const icons = classicRating.querySelectorAll('.srfm-classic-event [data-te-rating-icon-ref]');
        const ratingResult = classicRating.querySelector('.srfm-rating-field-result');
...
function handleSingleRatingClick(e) {
    const onStar = parseInt(e.target.closest('.srfm-rating-icon').getAttribute('data-value'));
    ...
    for (let i = 0; i < stars.length; i++) {
        stars[i].querySelector('svg').classList.remove('srfm-fill-current');
    }
