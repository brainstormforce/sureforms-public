...
class Helper {
	 ...
	 *
	 * @param int  $form_id The ID of the form.
	 * @param bool $check_for_live_mode Optional. Whether to check for live mode. Default true.
	 * @since x.x.x
	 * @return bool True if form is restricted, false otherwise.
	 */
	public static function is_form_restricted( $form_id, $check_for_live_mode = true ) {
		...
		/**
		 * Filter to control whether to check for live mode.
		 *
		 * @since x.x.x
		 */
		$check_for_live_mode = apply_filters( 'srfm_is_form_restricted_check_for_live_mode', $check_for_live_mode, $form_id );
	...
	public static function has_time_limit_reached( $form_restriction ) {
		...
		$hours    = self::get_string_value( $form_restriction['hours'] ?? '12' );
		$minutes  = self::get_string_value( $form_restriction['minutes'] ?? '00' );
		$meridiem = self::get_string_value( $form_restriction['meridiem'] ?? 'AM' );
		...
		$date_timestamp = self::get_timestamp_from_string( $date, $hours, $minutes, $meridiem );
			...
			<div class="srfm-form-container srfm-form-restriction-wrapper">
				<div class="srfm-form-restriction-message" role="alert" aria-live="assertive">
					<span class="srfm-form-restriction-icon" aria-hidden="true">
						<?php
							echo wp_kses(
								self::fetch_svg( 'instant-form-warning', 'srfm-form-restriction-icon', 'aria-hidden="true"' ),
								self::$allowed_tags_svg
							);
						?>
					</span>
					<p>
	 ...
	 */
	public static function get_timestamp_from_string( $date, $hours, $minutes, $meridiem ) {

		if ( empty( $date ) || ! is_string( $date ) || empty( $hours ) || ! is_string( $hours ) || empty( $minutes ) || ! is_string( $minutes ) || empty( $meridiem ) || ! is_string( $meridiem ) ) {
