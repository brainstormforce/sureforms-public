...
class Multichoice_Markup extends Base {
	...
	public function classic_styling( $attributes ) {
			...
			<div class="srfm-multi-choice-container srfm-main-container srfm-frontend-inputs-holder ' . esc_attr( $classname ) . '" id="srfm-multi-choice-container-' . esc_attr( $block_id ) . '">
				...
				<div class="srfm-radio-buttons flex flex-wrap mt-2">';
		if ( is_array( $options ) ) {
			foreach ( $options as $i => $option ) {
						...
						<div class="flex items-start srfm-classic-radio-btn srfm-classic-multi-choice">
							...
							<div> 
								<article id="srfm-multi-choice-option-' . esc_attr( $block_id . '-' . $i ) . '" class="text-sm font-medium leading-6 text-primary_color mt-[-0.5px]">' . esc_html( $option['optiontitle'] ) . '</article>
							</div>
						</div>
						</label>';
			}
