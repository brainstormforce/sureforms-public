 ...
 * @since 0.0.1
 */
class SureForms_Password_Markup {
	use Get_Instance;
	 ...
	 */
	public static function password_default_styling( $attributes ) {
		...
		'<label for="sureforms-input-password-' . esc_attr( $id ) . '" class="text-primary">' . esc_html( $label ) . ' ' . ( $required && $label ? '<span style="color:red;"> *</span>' : '' ) . '</label>' .
		'<input name="' . esc_attr( str_replace( ' ', '_', $label . 'SF-divider' . $id ) ) . '" id="sureforms-input-password-' . esc_attr( $id ) . '" type="password" area-required="' . esc_attr( $required ? 'true' : 'false' ) . '" placeholder="' . esc_attr( $placeholder ) . '" class="sureforms-input-field">' .
		...
		( '' !== $help ? '<label for="sureforms-input-password" class="text-secondary sforms-helper-txt">' . esc_attr( $help ) . '</label>' : '' ) .
	 ...
	 * @return string|boolean
	 */
	public static function password_classic_styling( $attributes ) {
		$id                  = isset( $attributes['id'] ) ? strval( $attributes['id'] ) : '';
		$required            = isset( $attributes['required'] ) ? $attributes['required'] : false;
		$placeholder         = isset( $attributes['placeholder'] ) ? $attributes['placeholder'] : '';
		$label               = isset( $attributes['label'] ) ? $attributes['label'] : '';
    ...
    <div class="relative mt-2">
        <input type="password" name="' . esc_attr( str_replace( ' ', '_', $label . 'SF-divider' . $id ) ) . '" id="sureforms-input-password-' . esc_attr( $id ) . '" area-required="' . esc_attr( $required ? 'true' : 'false' ) . '" placeholder="' . esc_attr( $placeholder ) . '" class="sf-classic-pwd-element">
        ...
        <div class="relative mt-2 rounded-md shadow-sm">
            <input type="password" id="sureforms-confirm-input-password-' . esc_attr( $id ) . '" area-required="' . esc_attr( $required ? 'true' : 'false' ) . '" placeholder="' . esc_attr( $placeholder ) . '" class="sureforms-confirm-input-password sf-classic-pwd-element">
            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-cen ter pr-3">
                <svg  class="h-5 w-5 text-gray-400" viewBox="0 0 30 30" fill="currentColor" aria-hidden="true">
                    <path d="M 15 2 C 11.145666 2 8 5.1456661 8 9 L 8 11 L 6 11 C 4.895 11 4 11.895 4 13 L 4 25 C 4 26.105 4.895 27 6 27 L 24 27 C 25.105 27 26 26.105 26 25 L 26 13 C 26 11.895 25.105 11 24 11 L 22 11 L 22 9 C 22 5.2715823 19.036581 2.2685653 15.355469 2.0722656 A 1.0001 1.0001 0 0 0 15 2 z M 15 4 C 17.773666 4 20 6.2263339 20 9 L 20 11 L 10 11 L 10 9 C 10 6.2263339 12.226334 4 15 4 z"></path>
                </svg>
            </div>
